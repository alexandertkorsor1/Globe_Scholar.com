...
export const FinanceWorkspace: React.FC = () => {
  ...
  const totalFeeRevenue = financialRecords.filter(f => f.record_type === 'registration_fee').reduce((acc, f) => acc + f.amount, 0);
  ...
  const handleCreateDisbursement = (e: React.FormEvent) => {
    ...
    financialRecords.push({
      ...
      currency: 'USD',
      status: 'approved',
      payment_reference: `PAY-FIN-${Math.floor(10000 + Math.random() * 90000)}`,
      approved_by_name: 'Fiona Gallagher (Finance)',
      notes: 'Approved via Finance ledger pipeline.',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
        <div className="glass-panel" style={{ padding: '18px' }}>
          ...
          <span style={{ fontSize: '0.7rem', color: '#64748b' }}>Verified via Portal Stripe Gateway</span>
        </div>
            ...
            <tbody>
              {visibleFinances.map(f => (
                <tr key={f.id}>
                  ...
                  <td><span className="badge badge-approved">{f.status.toUpperCase()}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="glass-panel animate-fade-in" style={{ width: '440px', padding: '24px', background: '#0f172a' }}>
            ...
            <form onSubmit={handleCreateDisbursement} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                ...
                <input type="text" value={dAppName} disabled style={{ width: '100%', padding: '8px 12px', borderRadius: '8px', background: 'rgba(255,255,255,0.05)', color: '#fff', border: '1px solid var(--border-color)' }} />
              </div>
              <div>
