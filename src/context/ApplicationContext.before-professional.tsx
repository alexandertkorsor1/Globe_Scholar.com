...
export const ApplicationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [institutionTasks, setInstitutionTasks] = useState<InstitutionTask[]>(INITIAL_INSTITUTION_TASKS);
  const [financialRecords, setFinancialRecords] = useState<FinancialRecord[]>(INITIAL_FINANCIAL_RECORDS);
  const [partnerUniversities, setPartnerUniversities] = useState<PartnerUniversity[]>(INITIAL_PARTNER_UNIVERSITIES);
  const [communications, setCommunications] = useState<Communication[]>(INITIAL_COMMUNICATIONS);
  const [auditLogs, setAuditLogs] = useState<AuditLog[]>(INITIAL_AUDIT_LOGS);
  const [statusHistory, setStatusHistory] = useState<ApplicationStatusHistory[]>(INITIAL_STATUS_HISTORY);
